"""core.py — VENDORED feature engine for the long-history pipeline.

THIS IS A FORK of v2/_shared.py, copied so that long_history/ is fully
self-contained: nothing in this folder imports from, or can affect, the
production model. Experiment here freely.

Differences from the production original:
  1. ANCHOR_YEARS extended to 2016-2030 (production ships 2024+ only).
  2. Moving-holiday anchors (CNY, Mid-Autumn, Dragon Boat, Ching Ming) listed
     back to 2016, each verified against chinese_calendar.
  3. CHING MING CORRECTED. Production hardcodes 04-04 every year; it is a
     solar term and actually falls on 05 April in 2018, 2019, 2022 and 2023.
  4. Data path points at the long-history CSV.

RE-SYNCING: if v2/_shared.py changes materially, diff it against this file.
A fork is the price of independence -- it will drift, so check periodically:
    diff <(sed -n '/^def /,$p' ../v2/_shared.py) <(sed -n '/^def /,$p' engine/core.py)
"""
...
ROOT = HERE.parent          # long_history/ -- this package owns everything below
DATA_DEMAND = ROOT / "data" / "raw" / "rawdata_long.csv"
...
ANCHOR_YEARS = range(2016, 2031)   # FORK: was range(2024, 2030)
...
HOLIDAY_ANCHORS = {
    ...
    # FORK: extended to 2016, verified against chinese_calendar holiday blocks
    ...
    "MidAutumn":  [pd.Timestamp(d) for d in [
        "2016-09-15", "2017-10-04", "2018-09-24", "2019-09-13", "2020-10-01",
        "2021-09-21", "2022-09-10", "2023-09-29",
        "2024-09-17", "2025-10-06", "2026-09-25"]],
    "DragonBoat": [pd.Timestamp(d) for d in [
        "2016-06-09", "2017-05-30", "2018-06-18", "2019-06-07", "2020-06-25",
        "2021-06-14", "2022-06-03", "2023-06-22",
        "2024-06-10", "2025-05-31", "2026-06-19"]],
    # FORK: Ching Ming is a SOLAR TERM -- 05 Apr in 2018/2019/2022/2023.
    # Production hardcodes 04-04 for every year, which is wrong for those.
    "ChingMing":  [pd.Timestamp(d) for d in [
        "2016-04-04", "2017-04-04", "2018-04-05", "2019-04-05", "2020-04-04",
        "2021-04-04", "2022-04-05", "2023-04-05",
        "2024-04-04", "2025-04-04", "2026-04-04"]],
    # --- COMPUTED ---
...
def check_holiday_coverage(demand: pd.DataFrame, holdout_days: int = HOLDOUT_DAYS,
    ...
    if gaps and verbose:
        ...
        print("             Those dates will be treated as ORDINARY days. "
              "Add the next occurrence to HOLIDAY_ANCHORS in engine/core.py.")
    return gaps
