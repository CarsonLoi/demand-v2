...
# ============================================================================
# v3 — Trend-aware features (Point 4)
# ============================================================================
def add_trend_features(df: pd.DataFrame, date_to_demand: dict) -> pd.DataFrame:
    """Long-window trend slopes + YoY growth at holiday anchors.

    Critical for capturing year-on-year demand growth that fixed lags miss.
    """
    target_dates = pd.to_datetime(df["target_date"]).to_numpy()
    horizons = df["horizon"].to_numpy().astype(int)
    n = len(df)

    # 1. Linear-regression slope over past 90/180 days (shift-safe)
    slope_90 = np.full(n, np.nan, dtype="float64")
    slope_180 = np.full(n, np.nan, dtype="float64")
    for i in range(n):
        T = pd.Timestamp(target_dates[i])
        ms = horizons[i] + 1
        # 90-day window
        vals_90 = []
        for k in range(90):
            v = date_to_demand.get(T - pd.Timedelta(days=ms + k), np.nan)
            if not (v is None or np.isnan(v)):
                vals_90.append((k, v))
        if len(vals_90) >= 30:
            xs = np.array([p[0] for p in vals_90])
            ys = np.array([p[1] for p in vals_90])
            slope_90[i] = -float(np.polyfit(xs, ys, 1)[0])  # negate so positive = growing
        # 180-day window
        vals_180 = []
        for k in range(180):
            v = date_to_demand.get(T - pd.Timedelta(days=ms + k), np.nan)
            if not (v is None or np.isnan(v)):
                vals_180.append((k, v))
        if len(vals_180) >= 60:
            xs = np.array([p[0] for p in vals_180])
            ys = np.array([p[1] for p in vals_180])
            slope_180[i] = -float(np.polyfit(xs, ys, 1)[0])
    df["trend_slope_90d"] = slope_90
    df["trend_slope_180d"] = slope_180

    # 2. 28-day-mean vs 84-days-ago 28-day-mean (mid-term growth)
    ratio_28_84 = np.full(n, np.nan, dtype="float64")
    diff_28_84 = np.full(n, np.nan, dtype="float64")
    for i in range(n):
        T = pd.Timestamp(target_dates[i])
        ms = horizons[i] + 1
        recent = [date_to_demand.get(T - pd.Timedelta(days=ms + k), np.nan) for k in range(28)]
        older = [date_to_demand.get(T - pd.Timedelta(days=ms + 56 + k), np.nan) for k in range(28)]
        r = np.array([v for v in recent if not (v is None or np.isnan(v))])
        o = np.array([v for v in older if not (v is None or np.isnan(v))])
        if len(r) >= 14 and len(o) >= 14 and o.mean() > 0:
            ratio_28_84[i] = r.mean() / o.mean()
            diff_28_84[i] = r.mean() - o.mean()
    df["growth_28_vs_84d"] = ratio_28_84
    df["growth_28_vs_84d_diff"] = diff_28_84

    # 3. Year-over-year growth at the matching DOW (most stable seasonal signal)
    yoy_growth_dow = np.full(n, np.nan, dtype="float64")
    for i in range(n):
        T = pd.Timestamp(target_dates[i])
        ms = horizons[i] + 1
        # Find 4 same-DOW values from the past, vs 4 same-DOW values from a year ago
        recent_dow = []
        ...
        for k in range(1, 13):
            ...
            if len(old_dow) >= 4:
                break
        if len(recent_dow) >= 2 and len(old_dow) >= 2 and np.mean(old_dow) > 0:
            yoy_growth_dow[i] = np.mean(recent_dow) / np.mean(old_dow)
    df["yoy_growth_same_dow"] = yoy_growth_dow

    return df


# ============================================================================
# v3 — Cross-holiday transfer features (Point 6)
# ============================================================================
def add_cross_holiday_features(df: pd.DataFrame, date_to_demand: dict) -> pd.DataFrame:
    """Pool patterns across CNY/Labour/GoldenWeek/MidAutumn — they all share
    the 'multi-day Mainland block' shape, so combining them gives more samples
    for the model to learn from.
    """
    # Already have mainland_block_length, mainland_block_day_index. Add:

    # 1. "Day-1 of a 4+ day Mainland block" (the highest-demand day usually)
    df["block_day1_4plus"] = (
        (df.get("mainland_block_day_index", 0) == 1)
        & (df.get("mainland_block_length", 0) >= 4)
    ).astype("int8") if "mainland_block_day_index" in df.columns else 0

    # 2. Day 1/2/3/last of any Mainland block (one-hot for short blocks)
    if "mainland_block_day_index" in df.columns and "mainland_block_length" in df.columns:
        for pos in [1, 2, 3]:
            df[f"block_day{pos}"] = (
                (df["mainland_block_day_index"] == pos)
                & (df["mainland_block_length"] >= 2)
            ).astype("int8")
        df["block_last_day"] = (
            (df["mainland_block_day_index"] == df["mainland_block_length"])
            & (df["mainland_block_length"] >= 2)
        ).astype("int8")

    # 3. Cross-holiday averaged historical "demand on this block-day-index"
    # For each block-day-index value (1..7), compute mean demand from past
    # holiday-block days at the same position.
    target_dates = pd.to_datetime(df["target_date"]).to_numpy()
    horizons = df["horizon"].to_numpy().astype(int)

    # Pre-build: map (block_day_idx, block_length_bucket) -> list of past demand
    # Use only training-style positions: all days where _is_mainland_workday is False
    # (we'll use the demand history available before each row's data cutoff)
    if "mainland_block_day_index" in df.columns:
        block_day_idx = df["mainland_block_day_index"].values
        block_length = df["mainland_block_length"].values

        expected_demand_at_pos = np.full(len(df), np.nan, dtype="float64")
        for i in range(len(df)):
            T = pd.Timestamp(target_dates[i])
            h = horizons[i]
            ms = h + 1
            cutoff = T - pd.Timedelta(days=ms)
            pos = block_day_idx[i]
            blen = block_length[i]
            if pos < 1 or blen < 2:
                continue
            # Look at past dates strictly before cutoff with same block position
            matches = []
            for k in range(1, 800):
                d = T - pd.Timedelta(days=k + ms - 1)  # only past
                wd = _is_mainland_workday(d)
                if wd is False:
                    info = _compute_block_info(d)
                    if info[1] == pos and info[0] >= 2:  # same position, multi-day block
                        v = date_to_demand.get(d, np.nan)
                        if not (v is None or np.isnan(v)):
                            matches.append(v)
                if len(matches) >= 5:  # cap for efficiency
                    break
            if matches:
                expected_demand_at_pos[i] = float(np.mean(matches))
        df["expected_demand_at_block_pos"] = expected_demand_at_pos

        # Ratio to recent baseline
        if "rolling_mean_28" in df.columns:
            df["block_pos_uplift_ratio"] = (
                df["expected_demand_at_block_pos"]
                / df["rolling_mean_28"].clip(lower=1)
            )
...
def build_matrix(demand: pd.DataFrame, holdout_days: int = HOLDOUT_DAYS) -> pd.DataFrame:
    ...
    print("  adding trend features (Point 4) ...")
    mat = add_trend_features(mat, date_to_demand)
    print("  adding cross-holiday features (Point 6) ...")
    mat = add_cross_holiday_features(mat, date_to_demand)
