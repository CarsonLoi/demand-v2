"""v2: LightGBM quantile + holiday upweight + Tier 1 features."""
    ...
    for h in range(1, HOLDOUT_DAYS + 1):
        ...
        for q in QUANTILES:
            m = lgb.LGBMRegressor(
                ...
                n_estimators=400, learning_rate=0.03, num_leaves=31, max_depth=7,
...
def main():
    print(f"=== v2 LightGBM-quantile (Tier 1 + reservations={USE_RESERVATIONS}) ===\n")
    demand = load_demand()
    ...
    print_metrics("LightGBM v2", metrics)

    pd.DataFrame(preds).assign(
        abs_error=lambda d: np.abs(d.y_true - d.y_p50),
        pct_error=lambda d: (d.y_true - d.y_p50) / d.y_true,
    ).to_csv(OUT / "simple_lightgbm.csv", index=False)
    with open(OUT / "simple_lightgbm_preds.pkl", "wb") as f:
        pickle.dump(preds, f)
    ...
    ax.plot(test_dates, preds["y_p50"], "s-", color="C3", markersize=4, label="P50")
    ax.fill_between(test_dates, preds["y_p10"], preds["y_p90"], color="C3", alpha=0.2)
    ax.axvline(train["date"].max(), color="red", linestyle="--", alpha=0.5)
    ax.set_title(f"v2 LightGBM-quantile | MAPE={metrics['mape']*100:.2f}%")
    ax.legend(loc="upper left"); ax.grid(alpha=0.3); plt.xticks(rotation=30); plt.tight_layout()
    plt.savefig(OUT / "simple_lightgbm.png", dpi=120, bbox_inches="tight"); plt.close()
