"""v2: CatBoost + holiday upweight + Tier 1 features."""
...
def main():
    print(f"=== v2 CatBoost (Tier 1 + reservations={USE_RESERVATIONS}) ===\n")
    demand = load_demand()
    ...
    print_metrics("CatBoost v2", metrics)

    pd.DataFrame(preds).assign(
        abs_error=lambda d: np.abs(d.y_true - d.y_p50),
        pct_error=lambda d: (d.y_true - d.y_p50) / d.y_true,
    ).to_csv(OUT / "simple_catboost.csv", index=False)
    with open(OUT / "simple_catboost_preds.pkl", "wb") as f:
        pickle.dump(preds, f)
    ...
    ax.plot(test_dates, preds["y_p50"], "s-", color="C6", markersize=4, label="P50")
    ax.fill_between(test_dates, preds["y_p10"], preds["y_p90"], color="C6", alpha=0.2)
    ax.axvline(train["date"].max(), color="red", linestyle="--", alpha=0.5)
    ax.set_title(f"v2 CatBoost | MAPE={metrics['mape']*100:.2f}%")
    ax.legend(loc="upper left"); ax.grid(alpha=0.3); plt.xticks(rotation=30); plt.tight_layout()
    plt.savefig(OUT / "simple_catboost.png", dpi=120, bbox_inches="tight"); plt.close()
